...
def main_control(dns, inacctday):
    try:
        ...
        logger.info('检查URL日志文件')
        res = ftp_hdfs()
        if res:
            for infilename in res:
                res2 = ftp_hdfs(
                    oscmd='python /root/PyEtl/etl_ftp.py ' + infilename)
...
def ftp_hdfs(hostip='202.119.86.2', hostuser='root', hostword='njust789!@#', oscmd='ls /data/ftp/'):
    try:
        # 创建SSH对象
